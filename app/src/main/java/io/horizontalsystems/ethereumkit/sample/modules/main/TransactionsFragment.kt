...
class ViewHolderTransaction(private val containerView: View) : RecyclerView.ViewHolder(containerView) {
        ...
        return eventsDecorations.map { event ->
            when (event) {
                is TransferEventDecoration -> {
                    val coin = io.horizontalsystems.ethereumkit.sample.Configuration.erc20Tokens.firstOrNull { it.contractAddress.eip55 == event.contractAddress.eip55 }?.name
                            ?: "n/a"
    ...
    private fun stringify(decoration: TransactionDecoration, transaction: TransactionRecord): String {
        val coinName = io.horizontalsystems.ethereumkit.sample.Configuration.erc20Tokens.firstOrNull { it.contractAddress.hex == transaction.to.address }?.name
                ?: "n/a"
    ...
    private fun stringify(token: SwapMethodDecoration.Token): String {
        return when (token) {
            ...
            is SwapMethodDecoration.Token.Eip20Coin -> io.horizontalsystems.ethereumkit.sample.Configuration.erc20Tokens.firstOrNull { it.contractAddress.eip55 == token.address.eip55 }?.code
                    ?: "n/a"
        }
    }
