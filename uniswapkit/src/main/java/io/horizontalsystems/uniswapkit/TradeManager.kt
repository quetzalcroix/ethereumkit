...
class TradeManager(
    ...
    fun pair(tokenA: Token, tokenB: Token): Single<Pair> {
        ...
        val (token0, token1) = if (tokenA.sortsBefore(tokenB)) Pair(
            tokenA,
            tokenB
        ) else Pair(tokenB, tokenA)
    ...
    fun transactionData(tradeData: TradeData): TransactionData {
        return buildSwapData(tradeData).let {
            TransactionData(to = routerAddress, value = it.amount, input = it.input)
        }
    }
    ...
    private fun buildSwapData(tradeData: TradeData): SwapData {
        ...
        val method = when (trade.type) {
            TradeType.ExactOut -> buildMethodForExactOut(
                tokenIn,
                tokenOut,
                path,
                to,
            ...
            TradeType.ExactIn -> buildMethodForExactIn(
                tokenIn,
                tokenOut,
                path,
                to,
    ...
    private fun buildMethodForExactOut(
        ...
        return when {
            tokenIn is Ether && tokenOut is Erc20 -> SwapETHForExactTokensMethod(
                amountOut,
                path,
                to,
                deadline
            )
            tokenIn is Erc20 && tokenOut is Ether -> SwapTokensForExactETHMethod(
                amountOut,
                amountInMax,
                path,
                to,
            ...
            tokenIn is Erc20 && tokenOut is Erc20 -> SwapTokensForExactTokensMethod(
                amountOut,
                amountInMax,
                path,
                to,
    ...
    private fun buildMethodForExactIn(
        ...
        return when {
            tokenIn is Ether && tokenOut is Erc20 -> SwapExactETHForTokensMethod(
                amountOutMin,
                path,
                to,
                deadline
            )
            tokenIn is Erc20 && tokenOut is Ether -> SwapExactTokensForETHMethod(
                amountIn,
                amountOutMin,
                path,
                to,
            ...
            tokenIn is Erc20 && tokenOut is Erc20 -> SwapExactTokensForTokensMethod(
                amountIn,
                amountOutMin,
                path,
                to,
