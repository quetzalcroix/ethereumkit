...
class EthereumKit(
    ...
    private val defaultGasLimit: Long = 21_000
    ...
    private fun rawTransaction(
        ...
        return (nonce?.let { Single.just(it) } ?: blockchain.getNonce()).let { nonceSingle ->
            nonceSingle.flatMap { rNonce ->
                Single.just(
                    RawTransaction(
                        gasPrice,
                        gasLimit,
    ...
    companion object {
        ...
        private fun getInstance(
            ...
            val userInternalTransactionsProvider =
                UserInternalTransactionSyncer(transactionProvider, transactionStorage)
            val transactionInternalTransactionSyncer =
                TransactionInternalTransactionSyncer(transactionProvider, transactionStorage)
            val outgoingPendingTransactionSyncer =
                PendingTransactionSyncer(blockchain, transactionStorage)

            val transactionSyncer = TransactionSyncer(blockchain, transactionStorage)

            val notSyncedTransactionManager =
                NotSyncedTransactionManager(notSyncedTransactionPool, transactionStorage)
            ...
            return EthereumKit(
                ...
                transactionSyncManager.set(this)
            }
        }
        ...
        private fun transactionProvider(
            ...
            return when (transactionSource.type) {
                is TransactionSource.SourceType.Etherscan -> {
                    val service = EtherscanService(
                        transactionSource.type.apiBaseUrl,
                        transactionSource.type.apiKey
                    )
                    EtherscanTransactionProvider(service, address)
                }
                is TransactionSource.SourceType.Memento -> EtherscanTransactionProvider(
                    EtherscanService(transactionSource.type.apiBaseUrl, ""),
                    address
                )
            }
        }
